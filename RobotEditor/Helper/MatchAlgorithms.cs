...
namespace RobotEditor.Helper
{
    public static class MatchAlgorithms
    {
        ...
        public static void DepthFirstSearch(IEnumerable<KeyValuePair<int, VoxelNode>> nodes, int level, int maxCycles, bool maxValue, bool maxLeafs, bool maxMax, int rotations, bool noGo, bool symmetry, double[] boundingBoxHalfExtents)
        {
            ...
            IEnumerable<VoxelNodeInner> robotMaxNodes = null;
            if (level < carTree.Level) // Check if leaf level of carTree is not yet reached
                robotMaxNodes = robotTree.GetAncestorNodes(relLevel).Cast<VoxelNodeInner>().Where(n => n != null).OrderByDescending(n => n.Max);
                ...
                if (level < carTree.Level)
                {
                    ...
                    if (maxLeafs)
                    {
                        ...
                        foreach (var robLeaf in robLeafs)
                        {
                            ...
                            maxLeafsValue += robLeaf.Value * maxLeafsListTemp.ElementAt(t).Value; // Best case would be, that the maximum leafs of the robot tree are combined with maximumum leafs of the carTree
                            t++;
                        }
        ...
        public static double MaxVoxelInit(IOrderedEnumerable<KeyValuePair<int, VoxelNode>> sortedNodes, int rotations, bool noGo, bool symmetry, double[] boundingBoxHalfExtents)
        {
            ...
            foreach (var node in sortedNodes) // Start with the largest cartTree node on the first level
            {
                diffBetweenRefs = carTree.CalculateNodePosition(node.Value);
                for (int i = 0; i < 3; i++)
                    diffBetweenRefs[i] = diffBetweenRefs[i] - robotRefNodePos[i];

                if (IsRobotInNoGoZone(diffBetweenRefs, robotTree.Level, boundingBoxHalfExtents) && noGo) // Check if robot would be moved to NoGo Zone
                    continue;

                if (IsRobotInSymmetryZone(diffBetweenRefs, robotTree.Level, boundingBoxHalfExtents) && symmetry) // Check if robot would be moved to Symmetry zone
                    continue;
        ...
        public static double MaxLeafInit(int rotations, double[] boundingBoxHalfExtents, bool noGo, bool symmetry)
        {
            ...
            for (int j = 0; j < rotations; j++)
            {
                ...
                var k = 0;
                var diffBetweenRefs = new int[3];
                foreach(var robotLeaf in orderedRobotLeafs) // Find the largest Robot leaf which doesn't lead to a movement that breaks the NoGo and Symmetry constraints
                {
                    ...
                    if (symmetry) // Check if robot would be moved into the NoGo or Symmetry Zone
                    {
                        if (noGo)
                        {
                    ...
                    else
                    {
                        if (noGo)
                        {
                            ...
                            break; // Check is this correct? Seems to always break;
                    }
                    
                    k++;
                }

                if (k >= orderedRobotLeafs.Count()) // No suitable robot leaf was found - check next rotation
                    continue;


                var largestRobotLeaf = orderedRobotLeafs.ElementAt(k); // Check is this correct? Variable is never used
                ...
                foreach (var robotLeaf in robotLeafs)
                {
                    ...
                    for (int i = 0; i < 3; i++)
                        robotLeafPos[i] = robotLeafPos[i] + diffBetweenRefs[i]; // Move robot leafs according to the found offset
